...
- **Clawd AI Action Tools**: Agentic loop via Anthropic Claude with 26 tools (13 lookup + 13 action). Includes SMS classification for intents like sick calls and client requests, triggering smart auto-responses. Lookup tools: workers, shifts, workplaces, shift requests, SMS logs, Discord alerts, available workers, Discord members, list calendar events, read recent Gmail emails, CRM workplaces, CRM shifts, CRM hotel requests. Action tools: send SMS, notify GM Lilee, send Discord, internal message, create shift request, create workplace (with auto-geocoding), update workplace, generate Replit prompt, create Google Calendar event, send Gmail email, create CRM hotel request, update CRM hotel request, update CRM confirmed shift.
...
- **Bidirectional CRM Sync**: Full two-way sync between WFConnect and Weekdays CRM. Inbound: CRM→App sync every 3 minutes for workplaces, confirmed shifts, hotel requests. Outbound: App→CRM push queue with exponential backoff retry (5 attempts max). TITO clock-in/out events auto-push to CRM. Webhook receiver at `POST /api/webhooks/crm` for real-time CRM event processing. Proactive Discord/SMS alerts on new hotel requests. Push queue health monitoring in AI monitor cycle. CRM Sync admin screen shows bidirectional status.
