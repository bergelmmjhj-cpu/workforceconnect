...
async function ensureWorkerApplicationsCompatibility() {
  try {
    ...
    await db.execute(sql`ALTER TABLE "worker_applications" ADD COLUMN IF NOT EXISTS "marketing_consent" boolean`);
    await db.execute(sql`ALTER TABLE "worker_applications" ALTER COLUMN "marketing_consent" SET DEFAULT false`);
    await db.execute(sql`UPDATE "worker_applications" SET "marketing_consent" = false WHERE "marketing_consent" IS NULL`);
    ...
    await db.execute(sql`ALTER TABLE "worker_applications" ADD COLUMN IF NOT EXISTS "sms_consent" boolean`);
    await db.execute(sql`ALTER TABLE "worker_applications" ALTER COLUMN "sms_consent" SET DEFAULT false`);
    await db.execute(sql`UPDATE "worker_applications" SET "sms_consent" = false WHERE "sms_consent" IS NULL`);
    await db.execute(sql`ALTER TABLE "worker_applications" ALTER COLUMN "sms_consent" SET NOT NULL`);
    await db.execute(sql`ALTER TABLE "worker_applications" ADD COLUMN IF NOT EXISTS "sms_consent_at" timestamp`);
    await db.execute(sql`ALTER TABLE "worker_applications" ADD COLUMN IF NOT EXISTS "application_source" text`);
    await db.execute(sql`UPDATE "worker_applications" SET "application_source" = 'Direct application' WHERE "application_source" IS NULL`);
    await db.execute(sql`ALTER TABLE "worker_applications" ADD COLUMN IF NOT EXISTS "assigned_recruiter" text`);
    await db.execute(sql`ALTER TABLE "worker_applications" ADD COLUMN IF NOT EXISTS "recruiter_notes" text`);
    await db.execute(sql`ALTER TABLE "worker_applications" ADD COLUMN IF NOT EXISTS "interview_stage" text`);
    await db.execute(sql`UPDATE "worker_applications" SET "interview_stage" = 'not_started' WHERE "interview_stage" IS NULL`);
    await db.execute(sql`ALTER TABLE "worker_applications" ADD COLUMN IF NOT EXISTS "interview_notes" text`);
    await db.execute(sql`ALTER TABLE "worker_applications" ADD COLUMN IF NOT EXISTS "deployment_readiness" text`);
    await db.execute(sql`UPDATE "worker_applications" SET "deployment_readiness" = 'not_ready' WHERE "deployment_readiness" IS NULL`);
    await db.execute(sql`ALTER TABLE "worker_applications" ADD COLUMN IF NOT EXISTS "payroll_readiness" text`);
    await db.execute(sql`UPDATE "worker_applications" SET "payroll_readiness" = 'not_ready' WHERE "payroll_readiness" IS NULL`);
    await db.execute(sql`ALTER TABLE "worker_applications" ADD COLUMN IF NOT EXISTS "missing_documents" text`);
    await db.execute(sql`ALTER TABLE "worker_applications" ADD COLUMN IF NOT EXISTS "next_recommended_action" text`);
    await db.execute(sql`ALTER TABLE "worker_applications" ADD COLUMN IF NOT EXISTS "document_request_sent_at" timestamp`);
    await db.execute(sql`ALTER TABLE "worker_applications" ADD COLUMN IF NOT EXISTS "last_contacted_at" timestamp`);
    ...
    await db.execute(sql`ALTER TABLE "applicants" ADD COLUMN IF NOT EXISTS "sms_consent" boolean`);
    await db.execute(sql`ALTER TABLE "applicants" ALTER COLUMN "sms_consent" SET DEFAULT false`);
    await db.execute(sql`UPDATE "applicants" SET "sms_consent" = false WHERE "sms_consent" IS NULL`);
    await db.execute(sql`ALTER TABLE "applicants" ALTER COLUMN "sms_consent" SET NOT NULL`);
    await db.execute(sql`ALTER TABLE "applicants" ADD COLUMN IF NOT EXISTS "sms_consent_at" timestamp`);
    await db.execute(sql`ALTER TABLE "applicants" ADD COLUMN IF NOT EXISTS "marketing_consent" boolean`);
    await db.execute(sql`ALTER TABLE "applicants" ALTER COLUMN "marketing_consent" SET DEFAULT false`);
    await db.execute(sql`UPDATE "applicants" SET "marketing_consent" = false WHERE "marketing_consent" IS NULL`);
    await db.execute(sql`ALTER TABLE "applicants" ALTER COLUMN "marketing_consent" SET NOT NULL`);
    await db.execute(sql`ALTER TABLE "applicants" ADD COLUMN IF NOT EXISTS "marketing_consent_at" timestamp`);
...
function configureExpoAndLanding(app: express.Application) {
  ...
  // Keep the simple apply form isolated from contractor onboarding routes.
  // Single subdomain-aware /apply route:
  //   apply.wfconnect.org → apply-form.html (standalone worker application)
  ...
  app.get("/", (req: Request, res: Response) => {
    ...
    // Apply subdomain — standalone public worker application form
    if (isApplySubdomain(req) && applyFormTemplate) {
      res.setHeader("Content-Type", "text/html; charset=utf-8");
      res.setHeader("Cache-Control", "no-cache");
      return res.status(200).send(applyFormTemplate);
